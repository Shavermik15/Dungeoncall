            ...
            try
            {
                ...
                switch (countfloor2)
                {
                    case 1:
                        Form3 form3 = new Form3(this);
                        form3.ShowDialog();
                        if (!StaticData.BattleStatus&& count==6)
                            this.Close();
                        else countfloor2++;
                        break;
                    case 2:
                        textBox1.Text = "Жажда наживы и приключений не позволила вам пройти мимо и вы с уверенностью идёте по тоннелю. Вы спокойно дошли до небольшой и тёмной комнаты. Внутри был только скелет какого-то человека. Вы решаете подойти поближе чтобы проверить вещи и внезапно в глазницы черепа загораються синим огнём и скелет начинает шевелиться. Вы в ужасе отпрыгиваете назад и не знаете что делать. ";
                        break; 
                }
